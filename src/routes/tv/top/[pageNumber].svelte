<script context="module">
  import { topTvUrl } from "../../_helper/urlGenerator.js";

  export async function preload(page, session) {
    const { pageNumber } = page.params;

    const r = await this.fetch(topTvUrl(pageNumber));
...
<section class="container">
  <h4 class="container__title">top rated TV series</h4>
  <div class="info__wrapper">
    <h5 class="info__page">page {page}</h5>
    {#if data.page !== data.total_page}
      <a rel="external" href={"tv/top/" + (page + 1)} class="info__next"
        >next</a
      >
    {/if}
  </div>
